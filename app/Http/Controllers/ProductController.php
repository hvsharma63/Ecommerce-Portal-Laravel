...
class ProductController extends Controller
{
    ...
    public function getFilterData(Request $request)
    {
        ...
        if(empty($request->getCategory) && empty($request->getColor))
        {
            $products = Product::where('status','Y')
            ->whereBetween('price',[$minValue,$maxValue])
            ->orderBy($request->getSort)
            ->get();
        }
        elseif (!empty($request->getCategory) && empty($request->getColor) ) {
            $products = Product::where('status','Y')
        ...
        if ($request->view == "list") {
            return view('layouts.product.frontend.listView')->with('products',$products);
        }
        else{
            return view('layouts.product.frontend.gridView')->with('products',$products);
        }
